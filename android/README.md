...
```
 boot / app update ─▶ BootReceiver ─▶ starts foreground service + daily sync
 app opened        ─▶ MainActivity ─▶ starts foreground service
                                         │
                                         ▼
 incoming SMS ─▶ SmsReceiver ─▶ WorkManager (expedited) ─┐
 (delivered even when app closed)                        ├─▶ POST /api/sms ─▶ backend ─▶ Telegram
 daily / heartbeat / "Sync now" ─▶ SyncWorker (sweep) ───┘
```
...
- The Home screen leads with a **status hero** (permission needed → add server →
  active), then grouped cards: *Always-on protection* (service + battery),
